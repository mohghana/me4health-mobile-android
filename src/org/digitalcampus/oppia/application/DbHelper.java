...
public class DbHelper extends SQLiteOpenHelper {
	...
	private static final String COURSE_C_TAG = "course_tag";
	private static final String COURSE_C_DATE = "date_time";
	...
	private static final String USER_C_STATUS= "status";
	private static final String USER_C_PHONENO = "phoneno";
	private static final String USER_C_PHONENO2 = "phoneno2";
	private static final String USER_C_PHONENO3 = "phoneno3";
	private static final String USER_C_SCHOOLCODE = "schoolcode";
	private static final String USER_C_YEARGROUP = "yeargroup";
	private static final String USER_C_PROGRAM = "program";
	private static final String USER_C_HOMETOWN = "hometown";
	private static final String USER_C_PASSWORD = "passwordencrypted";
	private static final String USER_C_SURVEY_STATUS = "survey_status";
	...
	public void createUserTable(SQLiteDatabase db){
		String sql = "CREATE TABLE ["+USER_TABLE+"] (" +
                ...
                "["+USER_C_STATUS +"] TEXT, " +
                "["+USER_C_PHONENO +"] TEXT, " +
                "["+USER_C_PHONENO2 +"] TEXT, " +
                "["+USER_C_PHONENO3 +"] TEXT, " +
                "["+USER_C_SCHOOLCODE +"] TEXT, " +
                "["+USER_C_YEARGROUP +"] TEXT, " +
                "["+USER_C_PROGRAM +"] TEXT, " +
                "["+USER_C_HOMETOWN +"] TEXT, " +
                "["+USER_C_SURVEY_STATUS +"] TEXT, " +
                "["+USER_C_APIKEY +"] TEXT " +
            ");";
		db.execSQL(sql);
	}
	...
	public long addOrUpdateCourse(Course course) {
		...
		values.put(COURSE_C_TAG, course.getCourseTag());
		values.put(COURSE_C_DATE, System.currentTimeMillis());
	...
	public long addOrUpdateUser(User user) {
		ContentValues values = new ContentValues();
		values.put(USER_C_USERNAME, user.getPhoneNo());
		...
		values.put(USER_C_PHONENO, user.getPhoneNo());
		values.put(USER_C_EMAIL, user.getEmail());
		values.put(USER_C_PROGRAM, user.getProgram());
		values.put(USER_C_STATUS, user.getStatus());
		values.put(USER_C_SCHOOLCODE, user.getSchoolCode());
		values.put(USER_C_YEARGROUP, user.getYeargroup());
		values.put(USER_C_HOMETOWN, user.getHometown());
		values.put(USER_C_APIKEY, user.getApiKey());
		values.put(USER_C_SURVEY_STATUS, user.getSurveyStatus());
		
		long userId = this.isUser(user.getPhoneNo());
		if (userId == -1) {
			Log.v(TAG, "Record added");
	...
	public long updateSurvey(long userId,String status) {
		ContentValues values = new ContentValues();
		values.put(USER_C_SURVEY_STATUS, status);
		
			db.update(USER_TABLE, values, USER_C_ID + "=" + userId, null);
			return userId;
	}
	// returns id of the row
		public long addOrUpdateUserOnProfileUpdate(User user) {
			ContentValues values = new ContentValues();
			values.put(USER_C_USERNAME, user.getPhoneNo());
			values.put(USER_C_FIRSTNAME, user.getFirstname());
			values.put(USER_C_LASTNAME, user.getLastname());
			values.put(USER_C_PHONENO, user.getPhoneNo());
			values.put(USER_C_PHONENO2, user.getPhoneNo2());
			values.put(USER_C_PHONENO3, user.getPhoneNo3());
			values.put(USER_C_EMAIL, user.getEmail());
			values.put(USER_C_PROGRAM, user.getProgram());
			values.put(USER_C_STATUS, user.getStatus());
			values.put(USER_C_SCHOOLCODE, user.getSchoolCode());
	...
	public ArrayList<Course> getAllCourses() {
		...
		while (c.isAfterLast() == false) {
			...
			course.setCourseTag(c.getString(c.getColumnIndex(COURSE_C_TAG)));
	...
	public ArrayList<Course> getCourseTags() {
		ArrayList<Course> courses = new ArrayList<Course>();
		String order = COURSE_C_ORDER_PRIORITY + " DESC, " + COURSE_C_TITLE + " ASC";
		Cursor c = db.query(COURSE_TABLE, null, null, null, COURSE_C_TAG, null, null);
		c.moveToFirst();
		while (c.isAfterLast() == false) {
	...
	public ArrayList<Course> getCoursesByTags(long userId,String courseTag) {
		ArrayList<Course> courses = new ArrayList<Course>();
		String order = COURSE_C_ORDER_PRIORITY + " DESC, " + COURSE_C_TITLE + " ASC";
		String s = COURSE_C_TAG + "=?";
		String[] args = new String[] { String.valueOf(courseTag) };
	...
	public ArrayList<User> getUserDetails(String username){
		...
		while (c.isAfterLast() == false) {
			...
			u.setPhoneNo2(c.getString(c.getColumnIndex(USER_C_PHONENO2)));
			u.setPhoneNo3(c.getString(c.getColumnIndex(USER_C_PHONENO3)));
			u.setStatus(c.getString(c.getColumnIndex(USER_C_STATUS)));
			u.setSchoolCode(c.getString(c.getColumnIndex(USER_C_SCHOOLCODE)));
			u.setSurveyStatus(c.getString(c.getColumnIndex(USER_C_SURVEY_STATUS)));
			users.add(u);
			c.moveToNext();
		}
		c.close();
	...
	public String getUserFirstName(String username){
		...
		while (c.isAfterLast() == false) {
			userFirstname = c.getString(c.getColumnIndex(USER_C_FIRSTNAME));
			c.moveToNext();
		}
		c.close();
